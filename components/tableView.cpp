...
void MainWindow::ShowAllComponents() {
    ...
    for (const component_record &record : recordsVector) {
        ui_->tableWidget->setRowCount(row + 1); // 动态增加行数
...
void MainWindow::ShowSomeComponents(const QVector<int>& exacIndex, const QVector<int>& fuzzyIndex) {
    ui_->tableWidget->clear(); // 清空表格
    int totalRows = exacIndex.size() + fuzzyIndex.size() + 2; // 总行数，包含两个空行
    ...
    for (int index : exacIndex) {
        if (index >= 0 && index < recordsVector.size()) { // 检查索引范围有效性
